...
import AvailableAt from '../movieDetails/AvailableAt';
...
const AppDetailsComponent = ({ isCreator = false }: { isCreator?: boolean }) => {
        ...
        axios
            ...
            .then(response => {
                ...
                setHeaderData({
                    ...
                    ratingCount: productData?.ratingCount && productData?.ratingCount + ' Downloads' || 'N/A',
                ...
                let mediaData: any = [];
                if (productData?.mediaLinks) {
                    mediaData = [...productData?.mediaLinks?.map((media: any) => media?.url)]; // Just the image URLs
                }

                if (productData?.trailerVideos) {
                    mediaData = [
                ...
                setProductInfo(productData?.productInfo?.Feature);
                setAdditionalInfoData1({
                    ...
                    productSize: '',
                    productInfo: {
                        Feature: productData?.productInfo.Feature
                    },
                ...
                <div className="lg:max-w-[80%] mx-auto py-3 md:px-4 md:py-8 769-1300:max-w-[100%]">
                    ...
                    <div className="flex flex-col md:flex-row md:space-x-8">
                        <div className={`w-full md:${isCreator ? 'w-full' : 'w-2/3'}`}>
                            ...
                            <RatingSection isCreator={isCreator} ratingData={ratingData}
                                productId={productId}
                            />
                            ...
                            {!isCreator && <div className="md:hidden mb-24 px-3">
                                <CategorySection
                                    // title="Apps"
                        ...
                        {!isCreator && <Sidebar
                            trendingData={filterRelatedData(trendingData)} recommendedData={recommendedData} />}
                    </div>
                </div>
            </div>
        </div>
